        ...
        <div class="list_warp">
          <div class="img_warp">
            <div v-for="(item,i) in imgList" v-show="imgList.length>0" :key="i" class="img_list" :class="{'bigImg':item.type===2}">
              <img v-lazy="$site+item.resUrl" @click="prevImg($site+item.resUrl)">
  ...
  methods: {
    ...
    onLoad () {
      ...
      getnews(this.params).then(res => {
        ...
        if (res.data.code === 1) {
          this.params.search = ''
          if (this.params.nowSize === 0) {
            this.imgList = []
            this.videoList = []
          }
          res.data.listThing.forEach(item => {
            if (item.thingResList.length > 0 && item.thingResList[0].type === 1) {
              this.imgList.push(item.thingResList[0])
            } else if (item.thingResList.length > 0 && item.thingResList[0].type === 2) {
