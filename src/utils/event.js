/*
 * @Author: your name
 * @Date: 2020-05-28 10:14:59
 * @LastEditTime: 2020-05-28 10:16:04
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \vue\src\utils\event.js
 */
/**
...
class VueTouch {
  ...
  constructor (el, binding, type) {
    ...
    const _listen = (type) => {
      return function (e) {
        const { stop, prevent, self, once } = g.binding.modifiers
  ...
  start (e) {
    ...
    const { x, y } = g.getMultiCenter(e.changedTouches)
    const _disX = x - g.firstTouchPosition.x
    const _disY = y - g.firstTouchPosition.y
    const _dis = Math.sqrt(_disX * _disX + _disY * _disY)
    const _timeDis = e.timeStamp - g.firstTouchTime
